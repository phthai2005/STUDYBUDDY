...
export default function ProfileScreen() {
  ...
  const handleMonthYearSubmit = () => {
    if (!applyMonthYearInput(monthYearInput)) {
      Alert.alert(
        "Không hợp lệ",
        "Nhập đúng định dạng MM/YYYY, ví dụ: 05/2026",
      );
  ...
  const toggleScheduleSlot = (day, slotKey) => {
    setSchedule((prev) => {
      ...
      return {
        ...prev,
        [day]: isActive
          ? daySlots.filter((item) => item !== slotKey)
          : [...daySlots, slotKey],
      };
    });
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Cập nhật thành công",
        "Đã lưu thông tin hồ sơ và lịch rảnh.",
      );
    } catch (error) {
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Chỉ số uy tín học tập</Text>
          <View
            style={{
              flexDirection: "row",
              alignItems: "center",
          ...
          >
            <Text style={{ fontSize: 40, marginRight: 10 }}>⭐️</Text>
            <View>
              <Text style={{ fontSize: 24, fontWeight: "bold" }}>
                {user.rating?.toFixed(1) || "0.0"}/5.0
              </Text>
              <Text style={{ color: "#8E8E93" }}>
                Dựa trên {user.ratingCount || 0} lượt đánh giá từ bạn bè
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.sectionSubtitle}>
            Chọn ca học bạn có thể tham gia để đề xuất người cùng lịch phù hợp.
          </Text>
                  ...
                  {SLOTS.map((slot) => {
                    ...
                    return (
                      ...
                      >
                        <View
                          style={[
                            styles.slotCellInner,
                            active && styles.slotCellInnerActive,
                          ]}
                        >
                          <Text
                            style={[
        ...
        >
          <Text style={[styles.saveButtonText, { color: "#FF3B30" }]}>
            Đăng xuất
          </Text>
        </TouchableOpacity>
      </ScrollView>
        ...
        <SafeAreaView style={styles.calendarModal}>
          <View style={styles.calendarContainer}>
            ...
            <View style={styles.calendarInputSection}>
              <Text style={styles.calendarInputLabel}>
                Nhập tháng/năm (MM/YYYY)
              </Text>
              <View style={styles.calendarInputRow}>
                ...
                />
                <TouchableOpacity
                  style={styles.calendarGoButton}
                  onPress={handleMonthYearSubmit}
                >
                  <Text style={styles.calendarGoButtonText}>Đi tới</Text>
                </TouchableOpacity>
              </View>
              <Text style={styles.calendarInputHint}>
                Dùng khi cần chọn ngày cách xa — không cần bấm {"<"} {">"} nhiều
                lần
              </Text>
            </View>
